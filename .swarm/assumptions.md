...
| A-0004 | No production error alerting exists; failures surface only when a user reports them or someone opens Vercel logs. | DevOps/SRE | 2026-08-23 | **OPEN — fix written, not yet deployed** | ops/uptime-monitor closes this, but it is not a monitor until it is deployed and verified (see its README). Still open until someone runs the four deploy commands and confirms an alert actually arrives. |
