...
app.get("/", async (req, res) => {
  try {
    ...
    res.send("ATG Apply Backend API (MySQL/MariaDB via Prisma 7 Adapter) is running");
  } catch (err) {
...
const startServer = async () => {
  try {
    ...
    await prisma.$queryRaw`SELECT 1`;
    systemLogger.info(`Database connected (${process.env.DB_NAME} @ ${process.env.DB_HOST}:${process.env.DB_PORT})`);
  } catch (err) {
